...
// Mock rules data for demo purposes
...
export const RuleProvider: React.FC<RuleProviderProps> = ({ children }) => {
  ...
  // API URL - in a real app, this would be your actual API endpoint
  ...
  const getRuleById = useCallback(async (id: string): Promise<Rule | null> => {
    try {
      ...
      // const response = await axios.get<Rule>(`${API_URL}/rules/${id}`);
      ...
      console.error('Error getting rule by ID:', err);
      setError('Failed to get rule. Please try again.');
      return null;
    } finally {
      setLoading(false);
    }
  ...
  const searchRules = useCallback(async (query: string): Promise<Rule[]> => {
    try {
      ...
      // In a real app, make an API call
      // const response = await axios.get<{ results: Rule[] }>(`${API_URL}/rules/search?q=${encodeURIComponent(query)}`);
      ...
      const results = mockRules.filter(rule => {
        return (
  ...
  const getCategories = useCallback(async (): Promise<string[]> => {
    try {
      ...
      // In a real app, make an API call
      // const response = await axios.get<{ categories: string[] }>(`${API_URL}/rules/categories`);
      ...
      setError('Failed to get categories. Please try again.');
      return [];
    } finally {
      setLoading(false);
    }
  ...
  const getRulesByCategory = useCallback(async (category: string): Promise<Rule[]> => {
    try {
      ...
      // In a real app, make an API call
      // const response = await axios.get<{ rules: Rule[] }>(`${API_URL}/rules/category/${encodeURIComponent(category)}`);
      ...
      setError('Failed to get rules by category. Please try again.');
      return [];
    } finally {
      setLoading(false);
    }
